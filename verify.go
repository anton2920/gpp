...
func (g GeneratorVerify) NamedType(r *Result, ctx GenerationContext, t *Type) {
	r.Printf("if err := %sVerify%s(l, &%s); err != nil {", t.PackagePrefix(), t.Name, ctx.VarName)
	{
		r.Line("return err")
	}
	r.Line("}")
...
func (g GeneratorVerify) Primitive(r *Result, ctx GenerationContext, lit TypeLit) {
	...
	case Int, Float:
		...
		if (len(vc.Min) > 0) && (len(vc.Max) > 0) {
			r.Printf("if (%s < %s) || (%s > %s) {", ctx.VarName, minConst.Name, ctx.VarName, maxConst.Name)
			{
				r.AddImport("fmt")
			...
			r.Printf("if %s < %s {", ctx.VarName, minConst.Name)
			{
				r.AddImport("fmt")
			...
			r.Printf("if %s > %s {", ctx.VarName, maxConst.Name)
			{
				r.AddImport("fmt")
	...
	case String:
		if (len(vc.MinLength) > 0) && (len(vc.MaxLength) > 0) {
			...
			r.Printf("if !strings.LengthInRange(%s, %s, %s) {", ctx.VarName, minLengthConst.Name, maxLengthConst.Name)
			{
				r.AddImport("fmt")
		...
		for _, fn := range vc.Funcs {
			if !strings.StartsWith(fn, "{") {
				fn = fmt.Sprintf("%s(l, %s)", fn, ctx.VarName)
			} else {
				fn = stdstrings.Replace(fn[1:len(fn)-1], "?", ctx.VarName, 1)
			}
